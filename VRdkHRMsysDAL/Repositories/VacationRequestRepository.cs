...
namespace VRdkHRMsysDAL.Repositories
{
    public class VacationRequestRepository : IVacationRequestRepository
    {
        ...
        {
            return await _context.VacationRequest.FirstOrDefaultAsync(ab => ab.VacationId.Equals(id));
        }

        public async Task<VacationRequest> GetByIdWithEmployeeWithTeamAsync(string id)
        {
            return await _context.VacationRequest.Include(req=>req.Employee).ThenInclude(emp=>emp.Team).FirstOrDefaultAsync(ab => ab.VacationId.Equals(id));
        }
